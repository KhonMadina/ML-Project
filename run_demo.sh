...
echo "[6/10] Training baseline model (char n-gram TF-IDF + Logistic Regression)..."
...
echo "[7/10] Single-text prediction (sanity check)..."
python -m modeling.predict \
  --model_dir "${MODEL_BASE}" \
  --text "អរគុណច្រើន សេវាកម្មល្��បំផុត 🙄"

echo "[8/10] Batch prediction and evaluation on test split..."
python -m modeling.predict \
  --model_dir "${MODEL_BASE}" \
  --input_csv "${DATA_DIR}/final_test.csv" \
  --output_csv "${DATA_DIR}/pred_test.csv"

echo "[9/10] Error analysis and reports (baseline)..."
python -m modeling.error_analysis \
  --model_dir "${MODEL_BASE}" \
  --input_csv "${DATA_DIR}/final_test.csv" \
  --output_dir "${REPORT_BASE}"

echo "[10/10] Completed all baseline steps."
echo
echo "Demo completed successfully (baseline model only)." 
